...
import { Chrome, ShieldCheck, UserCircle, ArrowRight, Loader2 } from 'lucide-react';
  ...
  const handleAction = async () => {
    ...
    try {
      if (mode === 'login') {
        ...
        // Registration — email + password only, no OTP
        if (!formData.email || !formData.password) {
          alert('אנא מלא אימייל וסיסמה');
          setLoading(false);
          return;
          ...
          <div className="space-y-4">
            {mode === 'login' && method === 'google' ? (
              ...
              <div className="space-y-6">
                ...
                >
                  {loading ? <Loader2 size={20} className="animate-spin mx-auto" /> : 'אמת והתחבר'}
                </button>
                <button 
                  onClick={() => setStep(1)}
              ...
              <>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-slate-900 transition-all"
                      />
                      <p className="text-xs text-slate-400 mt-1 mr-2">לפחות 8 תווים</p>
                    </div>
                  </>
                )}
                ...
                >
                  {loading ? <Loader2 size={20} className="animate-spin" /> : (mode === 'login' ? 'כניסה למערכת' : 'הרשמה והתחלה')}
