...
export class AuthService {
  ...
  async login(email, password) {
    ...
    try {
      ...
      if (!isPasswordHashed) {
        ...
        if (!isPasswordValid) {
          console.error('❌ [AuthService] Plain text password mismatch');
  ...
  async googleLogin(googleToken, mode = 'login', role = null) {
    ...
    console.log('🔵 [Service] googleLogin called - mode:', mode, 'role:', role);
    if (!googleToken) {
    ...
    try {
      ...
      if (!user) {
        // User doesn't exist - automatically create them (OAuth pattern: auto-register on first login)
        console.log('🔵 [Service] User does not exist, auto-creating new user via Google OAuth');
        // ALWAYS create new user with 'client' role and isApproved: false
