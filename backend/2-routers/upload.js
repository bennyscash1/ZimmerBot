...
// Create uploads directory if it doesn't exisזt
// In production: use /var/www/zimmerspro/backend/uploads
// In development: use backend/uploads
// const isProduction = process.env.NODE_ENV === 'production'; // ← חובה
const backendRoot = join(__dirname, '..');
const uploadsDir = join(backendRoot, 'uploads');  // /var/www/zimmerspro/backend/uploads (same for both)
...
router.post('/', authenticate, checkRole('admin', 'zimmer_owner', 'complex_owner', 'manager'), async (req, res, next) => {
  try {
    ...
    if (process.env.BASE_URL) {
      ...
      // Remove /api suffix if present (uploads are served at root level)
      ...
      // Build from request headers (works with proxies like nginx)
      ...
      // If host is localhost:3000 and we're in development, use relative URL
      // Otherwise use full URL
      if (host.includes('localhost:3000') || host.includes('127.0.0.1:3000')) {
        // In development, return relative URL so frontend can use its own origin
        // This allows the frontend to construct the correct URL based on its own origin
        fileUrl = `/uploads/${filename}`;
      } else {
        // In production, use full URL with protocol and host
        // This ensures the URL is absolute and can be accessed from anywhere
        fileUrl = `${protocol}://${host}/uploads/${filename}`;
      }
    }
    
    // IMPORTANT: Always return relative URL in production if nginx serves files
    // The frontend will fix the URL using fixImageUrl function
    // This is more reliable than trying to guess the correct URL on the backend
